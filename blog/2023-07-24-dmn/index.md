...
- CGW генерирует **HBT** с заданным интервалом. HBT мимикрирует под Gratuitous ARP (самообращенный запрос где IP адрес является адресом генератора запроса). По факту это фрейм L2 с некоторым вспомогательным payload, рассылаемый как multicast IP. 
...
- TTR, получив HBT через один или несколько входных портов - оценивает задержки и если по активному ППТра (в конкретный момент времени) HBT пришёл с превышением допустимого интервала - то происходит переключением на ППТра на следующий доступный ППТра . По факту TTR направляет исходящий трафик через порт, ассоциированный с ППТра. Таким образом управляется маршрут для восходящего трафика.

- Что бы информировать CGW о том, что надо сменить ППТра -  в его (CGW)  сторону незамедлительно отсылается Gratuitous ARP через активный в данный момент времени ППТра. Gratuitous ARP сам по себе не несёт смысловой нагрузки для промежуточного оборудования. CGW по получению Gratuitous ARP от TTR также переключает ППТра в зависимости от пути, которым пришёл ARP-пакет. Таким образом управляется маршрут для нисходящего трафика.
...
Необходимо уточнить, что переключается исходящий ППТра, и делается это не посредством изменения информации в каких-либо таблицах, по которым ведёт линейный или не линейный поиск. Мы просто в  глобальной переменной устанавливаем контекст исходящего порта ( в будущем пул портов ). Так и достигаются минимальные временные затраты на управление активным ППТра. 
