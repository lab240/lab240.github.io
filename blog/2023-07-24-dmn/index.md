...
Проект передачи данных вдоль линии ЖД впечатляет- это сотни километров оптоволокна, тысячи РРС (Радио-релейных станций), служба мониторинга и управления OSS/BSS, десятки поездов и бригад обеспечения на дрезинах). И наша небольшая, но сложная задача - **сделать подвижный поезд и все что в нем находится надежным "стационарным" сетевым клиентом**.
...
- **TTR (Train Track Router)**  - маршрутизатор, устанавливаемый на подвижно объекте (в нашем случае "поезде")

:boom: Далее довольно часто мы будем использовать термин  ППТр- расшифровывается **как Путь прохождения трафика**.
...
Для тестирования качественных показателей используются приборы BERcut-ETX и трафик-генератор T-Rex с различными профилями, что позволяет на разных профилях трафика оценить  количество потерь при переключении путей прохождения трафика.
...
На стационарном сервере или как мы говорим "на базе" (CGW) - программная реализация, генерирующего потока heart-beat,  с нашем же алгоритмом переключения ППТра, разработанным с использование DPDK
...
:::info Общая концепция FleaPath такова: 

- CGW генерирует хёрт-бит ("**HBT**") с заданным интервалом. HBT мимикрирует под Gratuious ARP (самообращенный запрос где IP адрес является адресом генератора запроса). По факту это фрейм L2 с некоторым вспомогательным payload, рассылаемый как multicast IP. 
  
- Настройками промежуточного оборудования обеспечивается прохождение фреймов с HBT через сетевую инфраструктуру, через пары РРС-земля ---- РРС-поезд и так далее, достигая портов TTR. 
  
- Каждый порт TTR ассоциирован с конкретным  ППТра, достигается это либо прямым подключением РРС в порт TTR либо через промежуточную сетевую инфраструктуру с использованием VLAN-ов.

- TTR, получив HBT через один или несколько входных портов - оценивает задержки и если по активному ППТра (в конкретный момент времени) HBT пришёл с превышением допустимого интервала - то происходит переключением на ППТра на следующий доступный ППТра . По факту TTR направляет исходящий трафик через порт, ассоциированный с ППТра порт.

- Что бы информировать CGW о том, что надо сменить ППТра -  в его (CGW)  сторону незамедлительно отсылается Gratuitous ARP через активный в данный момент времени ППТра. Gratuitous ARP сам по себе не несёт смысловой нагрузки для промежуточного оборудования. CGW по получению Gratuitous ARP от TTR также переключает ППТра в зависимости от пути, которым пришёл ARP-пакет.

:::

В общем случае при интервале HBT в 5000 микросекунд (5 миллисекунд) удаётся получить переключение исходящего ППТР-а в пределах 7000-10000 микросекунд (7-110 миллисекунд). При интервале 1 мс, мы получаем переключение в районе 1,5-3 мс, что является просто рекордным результатом. 

Меньше интервал рассылки HBT - выше реактивность в петле обратной связи, а значит большая фоновая нагрузка на сетевую инфраструктуру служебным трафиком. Однако при интервале HBT в 1000 микросекунд (1 миллисекунду), ширина канала пропускного трафика примерно 1Мбит\с, что при общей полосе 10Гбит\с совсем не много. 

Надо уточнить, что переключается исходящий ППТра, и делается это не посредством изменения информации в каких-либо таблицах, по которым ведёт линейный или не линейный поиск. Мы просто в  глобальной переменной устанавливаем контекст исходящего порта. Так и достигаются минимальные временные затраты на управление активным ППТра.

:::info А есть ли потери (все равно скажут, что потери есть) ?

О потерях. В стендовых испытаниях были диагностированы потери трафика, связанные как понятно с тем, что какой-то время ТТР отправлял фреймы по ППТра в никуда, в том смысле что связи между парой РРС  не было. Сколько пакетов, сколько трафика в объёме, может быть оценочно высчитано, исходя из интервала HBT и таймаута переключения. Прорабатывается несколько вариантов для повышения надёжности передачи по ППТра, исследования не закончены.

::: 

## Стенды и проверка алгоритма
