...
### Компьютер под управлением OpenSuse с китайской картой  на чипе Intel AX210
...
Получаем 122Мб\с что соответствует 122*8= **976Мбит\с**. 
...
Спустя месяц ожиданий к нам приехал еще один модуль Netgear, к удивлению он оказался упрощенным устройством для Mesh сети (Netgear Sattelite RBSE960). Он не богат на настройки и единственная его функция быть репитером основного роутера. Мало того, у него не  10Гбит проводного интерфейса, а только 2.5 и 1Гбит. 

![](img-wifi-mesh/mesh-interface-sattelite.jpg)

Как можно видеть, настроек немного (точнее, их совсем нет) !


### Проверим, что такое MESH сеть в понимании Netgear. 

Была собрана следующая схема. 

![](img-wifi-mesh/mesh-schm.jpg)

Настройка соединения в MESH происходит по "кнопке". То есть, мы жмем кнопку на роутере и на Сеттелите и они соединяются. Влиять на этот процесс нельзя. Убедиться, что соединение произошло, можно на основном роутере в разделе "присоединенные устройства".

![](img-wifi-mesh/mesh-connexted-sattelites.jpg)

Как видно из картинки, мы соединились на 5GHZ и никакие ухищрения не позволили мне соединить MESH на 6Ггц (отключал 5Ггц в настройках в обоих устроиствах). Будем проверять скорости в таком режиме, т.к. иного нам не дано.

- iperf3 в один поток

![](img-wifi-mesh/mesh-iperf3-1.jpg)

Видим 1,67Гбит\с, что очень достойно

- iperf3 в 8 потоков

![](img-wifi-mesh/mesh-iperf3-8.jpg)

Тест показывает 1,55Гбит\с что примерно также. 

- Скачаем большой файл через wget

![](img-wifi-mesh/mesh-wget.jpg)

Получаем 132Мб\с, что соответствует 1Гбит\с. Такую скорость мы часто уже видели на тесте скачивания файлов на 6Ггц и вообще все цифры похожи на то, что роутеры связаны на 6Ггц, но проверить это не представляется возможным.

### Впечатления про MESH 

Несмотря на откровенную убогость настроек, MESH от Netgear действительно работает. При разнесению на разные этажи бизнес-центра роутера и сеттелита, в удаленной точке скорость через MESH показала 30Мбит\с, хотя до этого на  репитере Ruijie было 10Мбит\с и стала намного стабильнее. Правда плата за это достаточно высока - стоимость Netgear в несколько раз выше. 

## Выводы

- Максимум, что может дать WiFi на 6.2Ггц это скорости передачи данных в диапазон 1,2-1,7Гбит, ближе к нижней границе. 
  
- Передача файлов происходит на более низкой скорости и сильно зависит от Linux, файловой системы, загрузки ядер и тп. 

- При тестах скачивания файла обязательно указывать что запись не идет в файл (иначе файловая система может сильно затормозить запись и тест в целом). Параметр `-O /dev/null` для wget обязателен.
...
- MESH сеть от NetGear порадовала простотой настройки и скоростью\стабильностью соединения, но не позволяет гибко управлять настройками соединений. 
